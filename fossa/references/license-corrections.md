...
- **Read-back shape**: `{ "scoped": { "licenses", "lastEditedBy", "updatedAt", "scope" }, "base": { "licenses", "justification", … } }`. `base` is FOSSA's automated conclusion (or a FOSSA-admin _global_ override when one exists) with its justification; `scoped` is the highest-priority conclusion visible at the requested scope **falling back to base** — so `scoped == base` does not prove no human conclusion exists. The discriminator is `scoped.scope`: `null` = fell back to the automated row, `global` = FOSSA admin, anything else = a conclusion at that scope (source: `modules/LicenseManager/conclusions.ts:163-246`, priority ladder automated < global < organization < project/release_group < revision/release).
- **There is no org-wide list endpoint** — `GET /api/license-conclusions` and `GET /api/v2/license-conclusions` both 404 (live-verified 2026-09-16). Auditing who concluded what across an org means the per-revision endpoint above per locator, or the database (`"RevisionLicenseConclusions"`: `scope IS NULL` = automated; any other scope = a scoped row). Project corrections can ALSO write org-scope conclusion rows as a side-effect, **but only when the correction removes a license**: stamped `'system'` when removing a detected license is what first creates the correction (`ProjectCorrection.fromLocator` called with `skipLicenses`, `models/projectcorrection/model.ts:253-261`), or with the correcting user's email/username when `ProjectCorrectionManager` deletes a single corrected license and no other corrected license shares its license id (`if (!areOtherLicensesForThisSignature)`, `modules/ProjectCorrectionManager/index.ts:580, 607-621`). Corrections that only add or edit licenses write none (traced at FOSSA `fe046cc9039a`; the `fromLocator` gate arrived in `0cf1253e36` — before it a newly created correction wrote the `'system'` row even when nothing was removed, so older rows can predate it). Either way, `last_edited_by` alone does not separate direct human conclusions from correction side-effects; cross-check `"ProjectCorrections"`.

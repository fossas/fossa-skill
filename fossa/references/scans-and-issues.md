...
- Scope the list server-side with `scope[type]=global|project|releaseGroup` (same scoping the FOSSA UI's issues page URL carries): `project` takes the **URL-encoded project locator** as `scope[id]`, optionally narrowed with `scope[revision]=` the **bare revision string** — the part after `$`, e.g. `scope[revision]=2.2.1`, not the full revision locator (both live-verified; passing the full locator gets a 404 `Project revision not found: {project}${project}${revision}`, because the server joins the two itself); `releaseGroup` takes numeric `scope[id]` + `scope[release]` — an "expected number, received NaN" validation error means your params matched that numeric branch (or `ids[]`/`teamId`), not that locators are unsupported. Filter by `status`, `severity` (e.g. `severity[]=critical`), package managers, labels, dates; `sort` supported.

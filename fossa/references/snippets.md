...
- A revision only has snippet data if the scan ran with snippet analysis: `fossa analyze --snippet-scan` (verify it ran by grepping analyze output for the fingerprint stats line, e.g. `unique_matched_files`). Upload success alone does not prove the snippet stage executed.
...
- Noise controls that matter at scale: contiguous-lines thresholds (imports/boilerplate-only matches are the main noise class) and the org/project auto-reject threshold.

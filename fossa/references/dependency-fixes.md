...
**Step B — confirm it resolved.** Read the dependencies list (`GET /api/v2/revisions/{REV}/dependencies`, paginate — `count` is clamped to [25, **100**] per page, `count=1000` still returns 100; the body carries `total`, so loop `page` until you've collected `total` — see `reports.md`) and check the added locator has licenses and `isUnknown: false`. Not every plausible correction resolves — a wrong Maven groupId can 200 on add and still be the wrong package. Probe the registry first (see `packages.md`) to raise the hit rate.
...
- **`git` dependencies and CVEs — write the GitHub owner and repository in lowercase.** Vulnerability matching on `git+` locators compares the `owner/repo` string case-sensitively against lowercase keys, so the canonical mixed-case spelling of a repository silently returns no CVEs. Live-verified 2026-09-21, same releases, both spellings resolving to the same commit: `github.com/Mbed-TLS/mbedtls` v2.28.0 → 0 vulnerability issues, `github.com/mbed-tls/mbedtls` → 9; `github.com/DMTF/libspdm` 2.3.1 → 0, `github.com/dmtf/libspdm` → 2. This affects every hand-written `git` name: `fossa-deps` entries, and locators added through `POST /api/revisions/{REV}/dependencies` (§2, `"kind": "locator"`), which stores `packageName` exactly as typed. `pkg:github/...` purls in an imported SBOM are the exception: they are lowercased on import (verified: `pkg:github/Mbed-TLS/mbedtls@v2.28.0` → `git+github.com/mbed-tls/mbedtls$…`). This pulls against the byte-identical replication advice in the previous bullet: when the source org's locator is mixed case, a faithful copy carries the missing CVEs with it, so lowercase it and accept the locator difference.
- `git` `version` takes a **tag or a commit SHA**; FOSSA resolves either to the commit (`git+github.com/openssl/openssl$<sha>`), and CVE matching follows git history, so an untagged commit between two releases gets the right list (verified with an untagged commit between `openssl-3.0.13` and `openssl-3.0.14`). Coverage is per upstream project, though — zero CVEs on a `git` dependency is not proof of a clean bill. Verified covered: openssl, zlib, mbedtls, libspdm, brotli, oniguruma, jansson. Verified NOT covered: BusyBox via its GitHub mirror, and repositories whose tags are dates (`tianocore/edk2` at `edk2-stable202311`), which never parse as versions. For those, import an SBOM component with a `cpe` instead (see `sbom-import.md`). CVE data for a revision FOSSA has never seen is computed after the first scan, so wait for the issue scan to report `SCANNED` and re-read before trusting a zero.
- apk/deb locator shape, when deriving entries from existing locators: `deb+<name>#<os>#<osVersion>$<arch>#<version>` — e.g. `deb+bash#debian#12$amd64#0:5.2.15-2+b9` (the epoch stays in `version`). rpm differs: `rpm-generic` takes a separate `epoch:` fossa-deps field that the CLI prepends as `<arch>#<epoch>:<version>` — don't copy an epoch into `version` for rpm (`fossa-cli src/App/Fossa/ManualDeps.hs:350-370`).

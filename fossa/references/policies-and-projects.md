...
POST /api/teams                # { "name", "autoAddUsers": false, "defaultRoleId", "uniqueIdentifier"? }   (documented)
...
(source: `routes/teams.ts` at FOSSA `22fc86d76b` — `GET /api/teams` :98-169, `POST /api/teams` :176-219 reads `name` / `autoAddUsers` / `defaultRoleId` (+ `uniqueIdentifier` — see the Lookups note below), `PUT /api/teams/:id` :278, `GET /api/teams/:id` :294-310, `PUT /api/teams/:id/projects` :816-951. Live-observed 2026-09-16: create, `?teamId=` filter, the 403 on a push-only token; everything else traced, not observed.)
...
- **Trap — `teamId` filters take the numeric id, never the `uniqueIdentifier`.** The `teamId` param on `GET /api/v2/projects` and the v2 issues endpoints accepts integers (or `null`). A numeric-looking unique identifier is therefore parsed as a team id: it **silently returns an empty result** (HTTP 200, `total: 0` / zero issue counts) — **or, if that number happens to be a real team's id, that other team's projects and issues** — never an error; only a non-numeric value is rejected with a 400. Resolve identifier → numeric `id` first, and treat a 200 as proof of nothing about the identifier.

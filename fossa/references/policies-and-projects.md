...
POST /api/teams                # { "name", "autoAddUsers": false, "defaultRoleId", "uniqueIdentifier"? }   (informal — the dashboard's route)
GET  /api/teams/{id}           # one team, plus teamProjects: [{ projectId, … }], teamUsers: [{ userId, roleId }], teamReleaseGroups
PUT  /api/teams/{id}           # rename / settings (name, autoAddUsers, defaultRoleId, uniqueIdentifier)
